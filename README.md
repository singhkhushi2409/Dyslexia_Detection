...
|---:|---|---|---|
| 1 | Ink Density (%) | $100 \times \frac{\#\{(x,y): B(x,y)>0\}}{H W}$ | Captures how much of the page is occupied by foreground ink. It can reflect writing compactness and instrument or scan conditions; unusually high or low values may accompany inconsistent motor execution, but are highly sensitive to cropping and pen choice. |
| 2 | Connected Components | $N = \#\{i: a_i \ge 20, h_i \ge 5, w_i \ge 2\}$ | Counts separated foreground regions after cleaning. Fragmentation, disconnected marks, or segmentation into many pieces can reflect irregular formation, though printed text, punctuation, and image noise also change this count. |
...
| 13 | Overlap / Collision (%) | $100 \times \frac{\#\{i: x_{i+1}<x_i+w_i\}}{N-1}$ | Counts adjacent x-sorted bounding boxes whose horizontal extents overlap. Collisions can signal crowded or joined writing, but touching letters, cursive writing, and imperfect connected-component segmentation can produce the same result. |
...
|-- packages.txt                # System package hint for hosted environments
|-- data/                       # Source and intermediate data assets
|-- images/                     # Optional application imagery
`-- model_training/             # Training notebook and split datasets
```
...
`packages.txt` contains `default-jre`, which may be required by optional language-tooling dependencies in the broader application. It is not required by the OpenCV feature extractor itself.
...
| Language-tooling installation fails | Optional dependencies may need Java or platform-specific components. | Install the runtime listed in `packages.txt`, then retry. Handwriting inference still depends on the Python packages in `requirements.txt`. |
